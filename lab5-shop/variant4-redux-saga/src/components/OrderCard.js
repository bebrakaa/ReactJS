      ...
      {isOpen ? (
        <div id={`order-details-${order.id}`}>
          ...
          <div className="order-details">
            <p>
              <strong>Адрес доставки:</strong> {order.shippingAddress.city}, {order.shippingAddress.address}
            </p>
            <p>
              <strong>Способ оплаты:</strong> {paymentLabels[order.paymentMethod] || order.paymentMethod}
            </p>
          </div>
        </div>
      ) : null}
